...
namespace SpaceInvaders
{
    public abstract class GameObject : Component
    {
        ...
        {
            int currentScore = SceneContext.GetScoreInt();
            int highScore = SceneContext.GetHighScoreInt();
            if (currentScore > highScore)
            {
                SceneContext.SetHighScore(currentScore);

                Font highScoreFont = FontMan.Find(Font.Name.HighScore);
                highScoreFont.UpdateMessage(currentScore.ToString());
            }
        }
